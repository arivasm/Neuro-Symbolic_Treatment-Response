...
ex:lower_effectiveness	rdf:type	ex:LowerEffectiveness .\n"""
    ...
    if treatment.effective_2:
        treatment_class = treatment_class + """<http://example/Treatment/""" + treatment.name + """>\tex:belong_to\tex:effective .\n"""
        # treatment_class = treatment_class + """<http://example/Treatment/""" + treatment.name + """>\tex:no_belong_to\tex:low_effect .\n"""
    if treatment.decrease_effectiveness_2:
        treatment_class = treatment_class + """<http://example/Treatment/""" + treatment.name + """>\tex:belong_to\tex:low_effect .\n"""
        # treatment_class = treatment_class + """<http://example/Treatment/""" + treatment.name + """>\tex:no_belong_to\tex:effective .\n"""
    ...
    for d in drug_list:
        ...
        for c in cls:
            if c == 'HigherToxicity':
                graph_ttl = graph_ttl + """treatment_drug:""" + treatment_drug + """\tex:related_to\tex:higher_toxicity .\n"""
            else:
                graph_ttl = graph_ttl + """treatment_drug:""" + treatment_drug + """\tex:related_to\tex:lower_effectiveness .\n"""
...
def get_ddi_triples(treatment, g):
    ...
    for index, row in g.iterrows():
        ...
        # graph_ttl = graph_ttl + """ddi:""" + treatment.name + row['precipitantDrug'] + row[
        #     'objectDrug'] + """\tex:ddiEffect\tex:""" + row['Effect_Impact'] + """ .\n"""

        # graph_ttl = graph_ttl + """<http://example/Treatment/""" + treatment.name + """>\tex:hasInteraction\tddi:""" + \
        ...
        graph_ttl = graph_ttl + """ddi:""" + treatment.name + row['precipitantDrug'] + row[
            'objectDrug'] + """\tex:related_to\t<http://example/Drug/""" + row['precipitantDrug'] + """> .\n"""
        graph_ttl = graph_ttl + """ddi:""" + treatment.name + row['precipitantDrug'] + row[
            'objectDrug'] + """\tex:related_to\t<http://example/Drug/""" + row['objectDrug'] + """> .\n"""
    return graph_ttl
